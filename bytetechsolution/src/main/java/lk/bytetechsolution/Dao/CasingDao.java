...
public interface CasingDao extends JpaRepository<CasingEntity,Integer>{
    ...
    @Query(value="SELECT * FROM bytetechsolution.casing cas WHERE " +"(?1 IS NULL OR cas.id = ?1) AND " +"(?2 IS NULL OR cas.itemcode = ?2) AND " +"(?3 IS NULL OR cas.warranty = ?3) AND " +"(?4 IS NULL OR cas.casematerial_id = ?4) AND " +"(?5 IS NULL OR cas.casecolor_id = ?5) AND " +"(?6 IS NULL OR cas.motherboardformfactor_id = ?6) AND " +"(?7 IS NULL OR cas.width = ?7) AND " +"(?8 IS NULL OR cas.depth = ?8) AND"+"(?9 IS NULL OR cas.height = ?9) AND" +"(?10 IS NULL OR cas.brand_id = ?10)",nativeQuery=true)
