...
public interface PowerSupplyDao extends JpaRepository<PowerSupplyEntity,Integer>{
    ...
    @Query(value="SELECT * FROM bytetechsolution.powersupply pwr WHERE " +"(?1 IS NULL OR pwr.id = ?1) AND " +"(?2 IS NULL OR pwr.itemcode = ?2) AND " +"(?3 IS NULL OR pwr.wattage = ?3) AND " +"(?4 IS NULL OR pwr.efficiency_id = ?4) AND " +"(?5 IS NULL OR pwr.modulerity_id = ?5) AND " +"(?6 IS NULL OR pwr.powersupplyformfactor_id = ?6) AND " +"(?7 IS NULL OR pwr.warranty = ?7) AND " +"(?8 IS NULL OR pwr.brand_id = ?8)",nativeQuery=true)
